<footer id="footer" class="footer dark-background">

    <div class="container footer-top">
      <div class="row gy-4">
        <div class="col-lg-4 col-md-12 footer-about">
          <a href="index.html" class="logo d-flex align-items-center">
            <span class="sitename">PPI</span>
          </a>
          <p></p>
          <p></p>
          <div class="social-links d-flex mt-4">
            <!-- <a href=""><i class="bi bi-twitter-x"></i></a>
            <a href=""><i class="bi bi-facebook"></i></a>
            <a href=""><i class="bi bi-instagram"></i></a>
            <a href=""><i class="bi bi-linkedin"></i></a> -->
            <!-- <a href="https://www.github.com/sduhoulab/" target="_blank"><i class="bi bi-github"></i></a> -->
          </div>
        </div>

        <div class="col-lg-2 col-6 footer-links">
          <h4>Useful Links</h4>
          <ul>
            <li><a href="index.php">Home</a></li>
            <li><a href="predictor.php">Predictor</a></li>
            <li><a href="#">Protome</a></li>
            <li><a href="#">About us</a></li>
          </ul>
        </div>

        <div class="col-lg-2 col-6 footer-links">
          <h4>Datasets</h4>
          <ul>
            <li><a href="#">Protome</a></li>
            <li><a href="#">Mutation</a></li>
          </ul>
        </div>

        <div class="col-lg-4 col-md-12 footer-contact text-center text-md-start"  style="display:none">
          <h4>Contact Us</h4>
          <p>The Center for Bioinformatics (CBI)</p>
          <p>National Institute of Health Data Science of China</p>
          <p>Shandong University, Shandong, P. R. China.</p>
          <p></p>
          <p><i class="bi bi-people d-flex align-items-center"><a href="#" style="padding-left:5px">Prof. Qingzhen Hou</a></i></p>
          <p><i class="bi bi-envelope d-flex align-items-center"><a href="#" style="padding-left:5px">houqingzhen@sdu.edu.cn</a></i></p>
        </div>

      </div>
    </div>

    <div class="container copyright text-center mt-4">
      <p>©<span>2026</span> <strong class="px-1 sitename">PPI</strong> <span>All Rights Reserved</span></p>
      <div class="credits">
        <!-- All the links in the footer should remain intact. -->
        <!-- You can delete the links only if you've purchased the pro version. -->
        <!-- Licensing information: https://bootstrapmade.com/license/ -->
        <!-- Purchase the pro version with working PHP/AJAX contact form: [buy-url] -->
        <!-- Powered by <a href=""></a> -->
      </div>
    </div>

  </footer>

  <!-- Scroll Top -->
  <a href="#" id="scroll-top" class="scroll-top d-flex align-items-center justify-content-center"><i class="bi bi-arrow-up-short"></i></a>

  <!-- Preloader -->
  <div id="preloader"></div>

  <script src="https://lf-cdn.coze.cn/obj/unpkg/flow-platform/chat-app-sdk/1.2.0-beta.19/libs/cn/index.js"></script>
<script>
function getCookieByName(name) {
    const match = document.cookie.match(new RegExp('(^| )' + name + '=([^;]+)'));
    if (match) {
        return match[2];
    }
    return null;
}
token_id = getCookieByName('PHPSESSID');

  new CozeWebSDK.WebChatClient({
    config: {
      bot_id: '7602086574654455860',
      isFrame: true,
    },
    componentProps: {
      title: 'PPI AI Agent Chat',
    },
    auth: {
      type: 'token',
      token: 'pat_Y9u7hySPFzXK6OSVY5iSLf2zmJHn7qlB6Em78ZJE1Nuo9NeOR8ygyG9piH3ESUyy',
      onRefreshToken: function () {
        return 'pat_Y9u7hySPFzXK6OSVY5iSLf2zmJHn7qlB6Em78ZJE1Nuo9NeOR8ygyG9piH3ESUyy'
      }
    },
    userInfo: {
      id: token_id,
      url: 'https://lf-coze-web-cdn.coze.cn/obj/coze-web-cn/obric/coze/favicon.1970.png',
      nickname: 'Visitor',
    },
    ui: {
      footer: {
        isShow: false,
        expressionText: 'Powered by ...',
      },
      chatBot:{
        isNeedAudio: false,
        uploadable: false,
      }
    }
  });
</script>
